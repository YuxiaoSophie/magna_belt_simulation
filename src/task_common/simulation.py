...
recording / ADMM.
Every solver parameter comes from ``defaults.py``; only the partition is scene-specific.
...
from task_common.joint_state import as_numpy, index_layout
...
class BeltTaskSimulation:
    ...
    def __init__(self, viewer: newton.viewer.ViewerBase, args: argparse.Namespace) -> None:
        ...
        self.vbd_iterations = int(getattr(args, "vbd_iterations", defaults.VBD_ITERATIONS))
        self.frame_id = 0
    ...
    def _apply_contact_materials(self, info: SceneInfo) -> None:
        """Global cable material for every shape, then the proxy override."""
        self.model.shape_material_ke.fill_(defaults.CABLE_CONTACT_KE)
        self.model.shape_material_kd.fill_(defaults.CABLE_CONTACT_KD)
        self.model.shape_material_mu.fill_(defaults.CABLE_CONTACT_MU)
        if not info.contact_override_shapes:
            return
        pad_idx = np.asarray(info.contact_override_shapes, dtype=np.int32)
        for array, value in (
            (self.model.shape_material_mu, defaults.GRIPPER_CONTACT_MU),
            (self.model.shape_material_ke, defaults.GRIPPER_CONTACT_KE),
            (self.model.shape_material_kd, defaults.GRIPPER_CONTACT_KD),
        ):
        ...
        return SolverCoupledProxy(
            ...
            entries=[
                SolverCoupled.Entry(
                    ...
                    solver=lambda v: SolverMuJoCo(
                        ...
                        iterations=defaults.MUJOCO_ITERATIONS,
                        ls_iterations=defaults.MUJOCO_LS_ITERATIONS,
            ...
            coupling=SolverCoupledProxy.Config(
                proxies=[SolverCoupledProxy.Proxy(
                    ...
                    mass_scale=defaults.PROXY_MASS_SCALE, mode=defaults.PROXY_COUPLING_MODE,
    ...
    def _capture_physics_graph(self) -> None:
        ...
        Capture records the launches without executing them, so the simulation state is
        untouched -- but the Python-level state_0/state_1 swap does run, hence the explicit
        restore.
        """
    ...
    def create_parser() -> argparse.ArgumentParser:
        ...
        parser.add_argument(
            "--vbd-iterations", type=int, default=defaults.VBD_ITERATIONS,
        ...
        # The scene's colliders are created with is_visible=False (defaults.py's
