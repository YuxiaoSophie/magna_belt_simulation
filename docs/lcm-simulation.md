...
| `POINT_CLOUD_CROPPED` | `drake::lcmt_point_cloud` | out | fields `x`/`y`/`z` FLOAT32 at 0/4/8, `rgb` UINT32 at 12 (bytes r, g, b, 0), `point_step` 16 | only with `--cameras --publish-point-cloud`: every `--perception-every` steps (15 = 0.075 s), right after that step's robot states, `utime` = the step's `FRANKA_STATE` utime; the collector's `pcd` (`cropped_point_cloud`: both ZEDs, crop `[0.2,-0.25,0.015]..[0.7,0.25,0.11]`, 5 mm voxels), world frame, unstructured (`height 1`, `width` = points), `IS_STRICTLY_FINITE`, little-endian, filler aligns the data to 16 B, as Drake's `PointCloudToLcm`; the cameras then render only on these steps (the scene's 20 fps camera period is ignored) |
| `RoundBeltState` | `magna::lcmt_round_belt_state` | out | `frame_name` `taskboard`, `num_control_points` 0, `num_points` 48 | only with `--publish-belt-state`: same steps and `utime` as `POINT_CLOUD_CROPPED`; the 48 belt body centres in the taskboard frame of `--board-params` (`task_board_position/orientation`, `round_belt_task.waypoints.board_pose`), float32 |
...
### Learned MPC with procman (`procman/newton_assembly_learned_sim.pmd`)
...
Script `run_learned_round_belt_sim`: stop everything, 1 s, sim, 15 s, group `simulation
subsystems` (`ur_control_simulation`, the UR and Franka OSCs), 2 s, `latent-encoder`
(group `perception`), 3 s, visualizer, assembly controller. `end_experiment` stops all four
groups (`debug` = `lcm-spy`, started by hand).

- **Wrappers.** `procman/run_in_magna_learned.sh` `cd`s to
  `${MAGNA_ROOT:-/home/hienbui/git/magna-deploy-learned-lcs}` (the deploy worktree: only its
  controller has `--local_lcm_url` and the learned branch; the main checkout's controller
  rejects that flag) and exports
  `LCM_DEFAULT_URL=${LEARNED_LCM_URL:-udpm://239.255.76.95:7695?ttl=0}`.
  `procman/run_learned_encoder.sh` runs `scripts/lcs/latent_encoder_node.py` from this repo's
  root. The sim reuses `procman/run_newton_sim.sh`.
- **URL policy.** The private group `udpm://239.255.76.95:7695?ttl=0` is a literal on every
  `exec` line: `--lcm-url` for the sim and the encoder, `--lcm_url` for the four magna binaries,
  plus `--local_lcm_url` for the controller (its debug channels follow it). `lcm-spy` has no
  flag, so its line is `/usr/bin/env LEARNED_LCM_URL=<url> run_in_magna_learned.sh ...`, which sets
  `LCM_DEFAULT_URL`. Never 7667. Under procman, every child inherits the deputy's own LCM
  socket, so `ss -ulnp` lists each child on the deputy's group as well. That socket belongs to
  the deputy, not to the child.
- **Sim flags.** `--cameras --publish-point-cloud --publish-belt-state --perception-every 15
  --board-params <worktree>/systems/parameters/round_belt_controller_params_learned_sim.yaml`
...
- **Encoder node.** `--deploy .../deploy_demo/deploy.npz --ur-state-channel UR_STATE_SIM
  ...
  `--belt-input points150`, but the model was trained on the 48 bodies (§8 row).
- **Gamepad gate.** The `pick` compiled segment of `round_belt_controller_params_learned_sim.yaml`
  has `user_confirm_to_run: false`: this stack has no gamepad, so a `true` gate never confirms.
- **Versus the lock-step harness** (`scripts/lcs/eval_learned_mpc.py`): nothing waits for
  anything. The sim paces to wall time, the encoder publishes whenever a matched cloud arrives,
  and the controller plans from the newest latent at its own tick. Latency therefore shows up as
  a stale state instead of a stalled sim. One continuous episode runs, so the controller is
  never restarted. A plan's `time_vec[0]` is the controller's context time when it consumed the
  latent (the newest `FRANKA_STATE`), not the latent's utime.

**Shell validation**: `scripts/lcs/run_learned_stack.py` (default URL `...96:7696`) launches
the same commands. It asserts, via procman's `sheriff_config.py`, that every pmd `exec` line
equals its own command table apart from the URL. It keeps the pmd's order but replaces the 15 s
and 3 s waits with the sim's first `FRANKA_STATE` and the encoder's first `LATENT_STATE`. It
takes an `ss -ulnp` snapshot of every started pid and fails on 7667 or any other multicast
group. It ends on `Switching to Terminate` + 2 s, `--max-s` or a dead child, stops its own
children in reverse order by pid, and classifies the recording (final 1 s, and the 1 s up to
`MPC completed!`). Results go to `data/lcs/stack_runs/<ts>-<label>/report.json`
(`--dry-run`: table + pmd check only).

Measured on 2026-09-24 (`data/lcs/stack_runs/20260924-134625-learned_stack1` and
`20260924-134817-learned_stack2`, `report.json`, the e2e winner's `learned_mpc` values):

| measured (`report.json`) | run 1 | run 2 |
|---|---|---|
| start-up: first `FRANKA_STATE` / first latent / controller (s) | 4.99 / 1.46 / 0.39 | 5.01 / 1.44 / 0.43 |
| latents: n, rate in sim time / wall (Hz), utime steps != 75 ms | 558, 13.33 / 8.98, 0 | 558, 13.33 / 9.10, 0 |
| encoder latency from `FRANKA_STATE` receipt, mean / p95 / max (ms) | 19.1 / 28.3 / 528 (first latent) | 17.6 / 27.0 / 45.1 |
| plan latency mean / p95 / max (ms); answered before the next latent; within 75 ms wall | 35.9 / 74.6 / 103; 80/80; 0.95 | 38.8 / 80.9 / 87.2; 80/80; 0.89 |
| plan `t0` minus latent utime, mean / max (ms) | 11.5 / 30 | 11.2 / 30 |
| sim real-time factor mean / min; compute max (ms); resyncs | 0.68 / 0.63; 41.0; 74 | 0.68 / 0.63; 48.2; 74 |
| marker t (sim s): pre-MPC 0 / MPC start / stage 0 / stage 1 | 8.17 / 33.95 / reached 33.95 / timeout 39.95 (dist 15.2) | 8.15 / 33.93 / reached 33.93 / timeout 39.94 (dist 15.1) |
| outcome, final 1 s and at `MPC completed!` | `outside` (belt 71 mm from the large pulley, on the board) | `outside` (66 mm) |

The sim runs at 0.68x real time with this stack (cameras + perception + viser + 5 magna
processes; compute mean 5.2 ms > the 5 ms step), so the latents are exactly one per 75 ms of sim
time, which is 9 Hz wall. The encoder keeps up (0 state-match misses, 0 rate-limited), and the
controller answers every MPC latent before the next one. The policy does not engage: the grasp
holds (hand 7 mm), stage 0 is skipped (tol 5), and stage 1 times out after 6 s with the belt
60-110 mm from the large pulley. The post-MPC release then drops it on the board. The
episode ends cleanly on the stage timeout (no hang), and `--max-s 150` stays as the bound.
...
| `--lcm-channels-file PATH` | none | a channels-override YAML (subset of the 19 `LcmChannels` fields, e.g. magna's own `/home/hienbui/git/magna/systems/parameters/lcm_channels.yaml`) |
...
| `--publish-point-cloud` | off | publish `POINT_CLOUD_CROPPED` (§2) every `--perception-every` steps; requires `--cameras`, which then render only on those steps; the `[STATS]` line gains `perception N x (render+cloud+publish mean/max ms, pts)` |
| `--publish-belt-state` | off | publish `RoundBeltState` (§2, taskboard frame) every `--perception-every` steps |
| `--perception-every N` | `15` (0.075 s) | control steps between perception messages |
| `--board-params PATH` | magna `round_belt_controller_params_sim.yaml` | yaml whose `task_board_position/orientation` define the `RoundBeltState` taskboard frame |
...
|---|---|---|
| `scripts/checks/check_lcmtypes.py` | the four packages resolve to `lcmtypes/<pkg>/`; then 17 vendored LCM types (incl. `drake::lcmt_point_cloud[_field]`, `magna::lcmt_round_belt_state`): fingerprint match against magna's own generated modules (skipped if that bazel tree is absent) + an encode/decode round trip | none |
...
| `scripts/checks/check_osc_backend.py` | 6 checks (`K0`-`K5`) of the lock-step OSC backend (magna's real Franka OSC as a child process on a private LCM URL), one shared sim: (K0) launch + warm-up handshake, (K1) 200 lock-step control steps under the hold (reply matching, effort/drift bounds), (K2) two snapshot restores (monotonic OSC clock, grasp, determinism), (K3) a 30 mm move driven by the waypoint commander, (K4) the timeout path (OSC stopped) and a restart, (K5) shutdown leaves no process behind | `--keep` |
| `scripts/checks/check_lcs_dataset.py` | 4 checks (`F0`-`F3`) of the `.npz` writer/validator (`task_common.lcs_dataset`) on synthetic data, no sim/GPU/LCM: (F0) format constants, (F1) `belt_points_ordered` material sampling (table vs the scene ellipse, fixed fractions under stretch, spline option), (F2) `EpisodeWriter` -> `validate_episode` round trip incl. the action-delta formula and `action_definition`, (F3) copies with a missing/malformed key (incl. an unknown `action_definition` / `belt_sampling`) are rejected naming that key; also validates a magna log if present (INFO) | none |
...
| `scripts/checks/check_grasp_variants.py` | 4 checks (`G0`-`G3`) of the grasp-varied start states (`scripts/lcs/make_grasp_variants.py`) on a private LCM URL (`...93:7693`): (G0) a temp 3-variant set (`--count 2`, seed 7, half the `set1` box) builds and its nominal `gv_00` matches `pre_place_1_osc.npz` (finger_tip / UR tracking within 1.5 mm / 1 deg, belt rms < 8 mm, measured offsets < 5 mm / 3 deg), (G1) every saved state restores into an OSC sim and holds 200 steps (belt drift < 5 mm) and `index.json` is complete, (G2) the `set1` probe's +-10 mm slides measure with the commanded sign and within 50 % (`[SKIP]` per slide that did not hold), (G3) no process on 7693, GPU apps at baseline, `pre_place_1*.npz` unchanged, temp set removed | `--keep` |
