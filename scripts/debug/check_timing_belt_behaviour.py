...
Run:
    uv run python scripts/debug/check_timing_belt_behaviour.py --model strip
    uv run python scripts/debug/check_timing_belt_behaviour.py --model strip --sweep
    uv run python scripts/debug/check_timing_belt_behaviour.py --model chain --num-elements 68 --device cuda:0
"""
...
def _write_sweep(rows, lines, rec, busy, busy_end, ceilings) -> None:
    ...
    text = "\n".join([
        ...
        "",
        f"Generated by `scripts/debug/check_timing_belt_behaviour.py --model strip --sweep` on {time.strftime('%Y-%m-%d %H:%M')}.",
        "",
