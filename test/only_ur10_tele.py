...
# SpaceMouse bridge reader (localhost:5005)
# Old Mac bridge method kept for reference. It is not used by default.
# class SpaceMouse:
#     def __init__(self, host=MAC_HOST, port=MAC_PORT):
#         self.ok = False
#         self.sock = None
#         self._buffer = b""
...
#             print(f"[SpaceMouse] Could not connect to {host}:{port}: {e}")
#             print("[SpaceMouse] Target will HOLD until the mouse bridge is up.")
#             self.ok = False
#
#     def read(self):
...
#                 if not chunk:
#                     print("[SpaceMouse] bridge disconnected.")
#                     self.ok = False
#                     return None
#                 self._buffer += chunk
...
#                 self._lin = np.array([msg.get("x", 0.0), msg.get("y", 0.0), msg.get("z", 0.0)],
#                                      dtype=np.float64)
#                 self._ang = np.array([msg.get("roll", 0.0), msg.get("pitch", 0.0), msg.get("yaw", 0.0)],
#                                      dtype=np.float64)
#                 self._buttons = list(msg.get("buttons", [0, 0]) or [0, 0])
#                 self._last_packet_time = time.perf_counter()
#                 got = True
#             except Exception as e:
#                 print(f"[SpaceMouse] bad packet: {e!r}")
#
#         now = time.perf_counter()
#         if got and now - self._last_debug >= 0.5:
...
#                 now - self._last_packet_time > INPUT_STALE_TIMEOUT):
