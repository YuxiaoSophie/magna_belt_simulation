...
:func:`utils.directives.load_directives` on that file, with the extension directives from
``round_belt_task.directives``, followed by the mapping of the loaded model ranges onto the
:class:`SceneInfo` index bookkeeping the solver partition needs.  The directive order fixes
every body and shape index.  No viewer and no solver here, and nothing touches the
finalized ``Model``: seeding the default joint state is ``joint_state.py``, running the
thing is ``simulation.py``.
"""
...
from utils.directives import ModelRecord, load_directives
...
def _span(records: Sequence[ModelRecord], what: str) -> tuple[list[int], list[int], list[int]]:
