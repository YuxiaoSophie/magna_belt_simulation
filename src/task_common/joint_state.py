"""Seeding the finalized model with its default joint state.
...
from task_common.defaults import GRIPPER_DRIVE_KD, GRIPPER_DRIVE_KE, GRIPPER_EFFORT_LIMIT
...
def find_gripper_driver_indices(model, start_joint_index, end_joint_index=None):
    """Coord and dof indices of the ``driver_joint`` joints in ``[start, end)``, sorted."""
...
def index_layout(length: int, n_coords: int, n_dofs: int, what: str) -> str:
...
def apply_default_joint_state(
    ...
    *,
    ...
    arm_defaults: Sequence[float],
    finger_labels: Sequence[str],
    finger_defaults: Sequence[float],
) -> None:
    """Seed ``model.joint_q`` with the Drake defaults and set position gains.

    Every index is resolved by joint label, not by ordinal position.  The resolved indices
    land in ``info.joint_config`` for ``BeltTaskSimulation._seed_control_targets`` /
    ``.test_final``.
    """
    ...
    arm_defaults = list(arm_defaults)
    arm_coord, arm_dof = coord_dof_lists(arm_labels)
    finger_coord, finger_dof = coord_dof_lists(finger_labels)
    ...
    gripper_coord, gripper_dof = find_gripper_driver_indices(
    ...
    for c, v in zip(arm_coord, arm_defaults):
        joint_q[c] = v
    for c, v in zip(finger_coord, finger_defaults):
        joint_q[c] = v
    ...
    layout = index_layout(target_len, n_coords, n_dofs, "model.joint_target_mode")
    if layout == "coord":
    ...
    for indices, ke, kd in (
        ...
        (grip_t, GRIPPER_DRIVE_KE, GRIPPER_DRIVE_KD),
    ):
        ke_np[indices] = ke
        kd_np[indices] = kd
    ...
    for d in gripper_dof:
        effort_np[d] = GRIPPER_EFFORT_LIMIT
