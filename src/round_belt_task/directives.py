...
from task_common.defaults import CABLE_CONTACT_KD, CABLE_CONTACT_KE, CABLE_CONTACT_MU
...
# Default belt density: a 22 g, 6.6 mm tube on a 248 x 168 mm ellipse (not the scene's rod size).
BELT_OUTER_MAJOR_DIAMETER = 0.248
BELT_OUTER_MINOR_DIAMETER = 0.168
BELT_RADIUS = 0.0066 * 0.5
TOTAL_BELT_MASS = 0.022


def estimate_belt_density() -> float:
    """Density [kg/m^3] of that tube; the ellipse perimeter is Ramanujan's approximation."""
...
def add_rod_ellipse(ctx: DirectiveContext, params: Mapping[str, Any]) -> None:
    ...
    (0.001), ``density`` (:func:`estimate_belt_density`) and ``ke``/``kd``/``mu``
    (``task_common.defaults.CABLE_CONTACT_*``).  Stores
    ...
    bodies, _joints = builder.add_rod(
        ...
        cfg=newton.ModelBuilder.ShapeConfig(
            density=num("density", estimate_belt_density()),
            ke=num("ke", CABLE_CONTACT_KE), kd=num("kd", CABLE_CONTACT_KD),
            mu=num("mu", CABLE_CONTACT_MU), margin=num("margin"), gap=num("gap")),
