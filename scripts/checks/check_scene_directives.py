...
Run:
    uv run python scripts/checks/check_scene_directives.py
    uv run python scripts/checks/check_scene_directives.py --device cpu
"""
...
from task_common.defaults import make_robust_table_collision_cfg, make_visual_cfg
...
# of scripts/checks/check_round_belt_task_poses.py on purpose).
# ----------------------------------------------------------------------------
...
ROBOTIQ_MJCF = REPO_ROOT / "assets/common/robotiq_2f85/2f85.xml"
...
def _load(path: Path, directives=None):
    ...
    return load_directives(
        ...
        directives=directives or {}, visual_cfg=make_visual_cfg(),
        collision_cfg=make_robust_table_collision_cfg(visible=False),
    )
