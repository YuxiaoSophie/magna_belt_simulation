...
Builds the Newton scene from the ``round_belt_task`` package, runs forward
...
the Drake yaml (``assets/round_belt_task/round_belt_scene.yaml`` names its
source files). Check 6 recomputes the Franka/UR10 chain FK
...
Run:
    uv run python scripts/checks/check_round_belt_task_poses.py
    uv run python scripts/checks/check_round_belt_task_poses.py --device cpu
"""
...
# the ``round_belt_task`` package.  Every check below compares the scene module
...
def check_scene_constants(ctx: dict) -> None:
    ...
    for name, tf, xyz, rpy_deg in (
        ...
        assert_close_tf(
            f"round_belt_task.{name} vs round-belt-scene.dmd.yaml",
            tf_to_mat4(tf),
