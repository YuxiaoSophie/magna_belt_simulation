...
UR10_BASE_YAW = -math.pi / 2.0 # robot 1: face -Y, toward table

# Robot 2 is mirrored to the opposite long side of the table.
UR10_2_BASE_X = UR10_BASE_X
UR10_2_BASE_Y = -UR10_BASE_Y
UR10_2_BASE_Z = UR10_BASE_Z
UR10_2_BASE_YAW = +math.pi / 2.0 # face +Y, toward table
...
# Robot 2 starts at the exact vertex opposite Robot 1.
# Robot 1 is at +X on the ellipse, so Robot 2 is at -X at the same Y/Z height.
BELT_GRASP_POS_2 = (
    BELT_CENTER_X - 0.5 * BELT_OUTER_MAJOR_DIAMETER,
    BELT_CENTER_Y,
    BELT_CENTER_Z,
...
BELT_APPROACH_POS_2 = (
    BELT_GRASP_POS_2[0],
    BELT_GRASP_POS_2[1],
    BELT_GRASP_POS_2[2] + BELT_APPROACH_CLEARANCE,
)
GRIPPER_TABLE_YAW_2 = -math.pi / 2.0
GRIPPER_DOWN_QUAT_2 = (
    math.cos(0.5 * GRIPPER_TABLE_YAW_2),
    math.sin(0.5 * GRIPPER_TABLE_YAW_2),
    0.0,
    0.0,
)
...
# Closing stays compliant so the belt can be detected without an impulsive pinch.
...
SHARED_PATH_2_DEFAULT = "/tmp/sm_teleop_target_2.bin"
...
def _add_ur10_and_gripper_assets(
    builder: newton.ModelBuilder, body_offset: int,
    base_xyz=(UR10_BASE_X, UR10_BASE_Y, UR10_BASE_Z),
    base_yaw=UR10_BASE_YAW,
) -> dict[str, Any]:
    base_pos = wp.vec3(float(base_xyz[0]), float(base_xyz[1]), float(base_xyz[2]))
    base_rot = wp.quat_from_axis_angle(wp.vec3(0.0, 0.0, 1.0), float(base_yaw))
...
class Example:
    ...
    def __init__(self, viewer, args):
        ...
        # ---------------- TWO UR10 + ROBOTIQ ROBOTS ----------------
        # Robot 1: original +Y-side arm.
        robot1_body_start = builder.body_count
        robot1_joint_start = builder.joint_count
        robot1_shape_start = builder.shape_count
        asset_info = _add_ur10_and_gripper_assets(
            builder, robot1_body_start,
            base_xyz=(UR10_BASE_X, UR10_BASE_Y, UR10_BASE_Z),
            base_yaw=UR10_BASE_YAW,
        )
        robot1_body_end = builder.body_count
        robot1_joint_end = builder.joint_count
        ...
        if len(self.gripper_proxy_bodies) != 2:
            raise RuntimeError(f"Robot 1 requires exactly 2 pad bodies; got {self.gripper_proxy_bodies}")
        ...
        robot1_shape_end = builder.shape_count

        self.robot1_bodies = list(range(robot1_body_start, robot1_body_end))
        self.robot1_joints = list(range(robot1_joint_start, robot1_joint_end))
        self.robot1_shapes = list(range(robot1_shape_start, robot1_shape_end))
        self._robot_joint_start = robot1_joint_start
        self._robot_joint_count = robot1_joint_end
        self._robot_tool_body_idx = asset_info["tool_body_idx"]
        self._robot_tcp_body_idx = _find_gripper_tcp_body(
            builder, asset_info["gripper_body_start"], asset_info["gripper_body_end"]
        )

        # Robot 2: mirrored to the -Y side and facing +Y.
        robot2_body_start = builder.body_count
        robot2_joint_start = builder.joint_count
        robot2_shape_start = builder.shape_count
        asset_info_2 = _add_ur10_and_gripper_assets(
            builder, robot2_body_start,
            base_xyz=(UR10_2_BASE_X, UR10_2_BASE_Y, UR10_2_BASE_Z),
            base_yaw=UR10_2_BASE_YAW,
        )
        robot2_body_end = builder.body_count
        robot2_joint_end = builder.joint_count

        self.gripper2_proxy_bodies = _select_gripper_proxy_bodies(
            builder, asset_info_2["gripper_body_start"], asset_info_2["gripper_body_end"]
        )
        if len(self.gripper2_proxy_bodies) != 2:
            raise RuntimeError(f"Robot 2 requires exactly 2 pad bodies; got {self.gripper2_proxy_bodies}")
        self.gripper2_pad_shapes = _replace_proxy_pad_colliders_with_two_planes(
            builder, self.gripper2_proxy_bodies
        )
        self.gripper2_proxy_shapes = list(self.gripper2_pad_shapes)
        robot2_shape_end = builder.shape_count

        self.robot2_bodies = list(range(robot2_body_start, robot2_body_end))
        self.robot2_joints = list(range(robot2_joint_start, robot2_joint_end))
        self.robot2_shapes = list(range(robot2_shape_start, robot2_shape_end))
        self._robot2_joint_start = robot2_joint_start
        self._robot2_joint_end = robot2_joint_end
        self._robot2_tool_body_idx = asset_info_2["tool_body_idx"]
        self._robot2_tcp_body_idx = _find_gripper_tcp_body(
            builder, asset_info_2["gripper_body_start"], asset_info_2["gripper_body_end"]
        )

        # The coupled MuJoCo entry owns BOTH robots.
        self.robot_bodies = self.robot1_bodies + self.robot2_bodies
        self.robot_joints = self.robot1_joints + self.robot2_joints
        self.robot_shapes = self.robot1_shapes + self.robot2_shapes
        self.robot_proxy_bodies = list(self.gripper_proxy_bodies) + list(self.gripper2_proxy_bodies)
        self.all_gripper_pad_shapes = list(self.gripper_pad_shapes) + list(self.gripper2_pad_shapes)
        robot_shape_end = robot2_shape_end

        print(f"[INFO] Robot 1 proxy pads: {self.gripper_pad_shapes}")
        print(f"[INFO] Robot 2 proxy pads: {self.gripper2_pad_shapes}")
        ...
        # Visual stands for both robot bases.
        for bx, by, bz in (
            (UR10_BASE_X, UR10_BASE_Y, UR10_BASE_Z),
            (UR10_2_BASE_X, UR10_2_BASE_Y, UR10_2_BASE_Z),
        ):
            builder.add_shape_cylinder(
                -1, xform=wp.transform(wp.vec3(bx, by, bz / 2.0)),
                half_height=bz / 2.0, radius=UR10_STAND_RADIUS, cfg=make_visual_cfg())
        ...
        if self.all_gripper_pad_shapes:
            mu_np[np.asarray(self.all_gripper_pad_shapes, dtype=np.int32)] = GRIPPER_CONTACT_MU
        ...
        if self.all_gripper_pad_shapes:
            pad_idx = np.asarray(self.all_gripper_pad_shapes, dtype=np.int32)
        ...
        rinfo = self._configure_robot_joints_range(
            self._robot_joint_start, self._robot_joint_count, "Robot 1"
        )
        rinfo2 = self._configure_robot_joints_range(
            self._robot2_joint_start, self._robot2_joint_end, "Robot 2"
        )
        # Give each free pulley axle real bearing behavior (viscous decay + Coulomb stop).
        ...
        self.gripper2_open_values = rinfo2["gripper_open_values"]
        self.gripper2_closed_values = rinfo2["gripper_closed_values"]
        ...
        self._initialize_robot2_at_approach()
        ...
        if ctrl_len == n_coords:
            ...
            self.arm2_targetq_indices = list(self.arm2_coord_indices)
            self.gripper2_targetq_indices = list(self.gripper2_coord_indices)
            ...
            self.arm2_targetq_indices = list(self.arm2_dof_indices)
            self.gripper2_targetq_indices = list(self.gripper2_dof_indices)
            ...
            for target_idx, coord_idx in zip(self.arm2_targetq_indices, self.arm2_coord_indices):
                base_targets[target_idx] = joint_q_np[coord_idx]
            for target_idx, open_val in zip(self.gripper2_targetq_indices, self.gripper2_open_values):
                base_targets[target_idx] = open_val
        else:
            raise RuntimeError(
        ...
        self.link2_index = int(self._robot2_tcp_body_idx)
        self.g2_open = list(self.gripper2_open_values)
        self.g2_closed = list(self.gripper2_closed_values)
        self.arm2_target_indices = list(self.arm2_targetq_indices)
        self.gripper2_target_indices = list(self.gripper2_targetq_indices)
        ...
        print(f"[INFO] seeded Robot 1 target buffer: {self.args.buffer}")

        # Robot 2 gets a completely independent target/gripper buffer.
        base2 = bq[self.link2_index]
        base2_pos = np.array(base2[0:3], dtype=np.float64)
        base2_quat = _norm4(np.array(base2[3:7], dtype=np.float64))
        tip2_0 = base2_pos + _rotate_vec(base2_quat, self.tip_offset)
        self._target2_pos = tip2_0.copy()
        self._target2_xyzw = base2_quat.copy()
        self._spacemouse2_raw_pos_prev = tip2_0.copy()
        self._grip2_fraction = 0.0
        self._grip2_requested_fraction = 0.0
        self._grip2_hold_fraction = None
        self._grip2_stall_frames = 0
        self._grip2_actual_fraction = 0.0
        self._grip2_actual_speed = 0.0
        self._arm2_cmd = np.array(
            [float(main_q[ci]) for ci in self.arm2_coord_indices], dtype=np.float64
        )
        self.shared2 = SharedTarget(self.args.buffer2)
        self.shared2.write(self._target2_pos, self._target2_xyzw, 0.0, ready=1.0)
        print(f"[INFO] seeded Robot 2 target buffer: {self.args.buffer2}")
    ...
    def _build_ik2(self) -> None:
        """Independent IK model for the mirrored second UR10."""
        ik_builder = newton.ModelBuilder(up_axis=newton.Axis.Z, gravity=-9.81)
        ik_info = _add_ur10_and_gripper_assets(
            ik_builder, body_offset=0,
            base_xyz=(UR10_2_BASE_X, UR10_2_BASE_Y, UR10_2_BASE_Z),
            base_yaw=UR10_2_BASE_YAW,
        )
        ik_tcp_body = _find_gripper_tcp_body(
            ik_builder, ik_info["gripper_body_start"], ik_info["gripper_body_end"]
        )

        self.ik2_model = ik_builder.finalize(device=self.device)
        self.ik2_arm_coord_indices, _ = find_n_actuated_indices(
            self.ik2_model, 0, UR10_ARM_DOFS
        )
        if len(self.ik2_arm_coord_indices) != len(self.arm2_coord_indices):
            raise RuntimeError(
                f"Robot 2 IK/main arm coordinate mismatch: "
                f"ik={self.ik2_arm_coord_indices}, main={self.arm2_coord_indices}"
            )

        ik_q_seed = self.ik2_model.joint_q.numpy().astype(np.float32).copy()
        main_q = self.model.joint_q.numpy().astype(np.float32)
        for ik_idx, main_idx in zip(self.ik2_arm_coord_indices, self.arm2_coord_indices):
            ik_q_seed[ik_idx] = main_q[main_idx]
        ik_grip_coord, _ = find_gripper_driver_indices(self.ik2_model, 0)
        for ik_idx, open_val in zip(ik_grip_coord, self.gripper2_open_values):
            ik_q_seed[ik_idx] = open_val

        self.ik2_joint_q = wp.array(ik_q_seed.reshape(1, -1), dtype=float, device=self.device)
        self.ik2_target_positions = wp.array(
            [wp.vec3(*BELT_APPROACH_POS_2)], dtype=wp.vec3, device=self.device
        )
        self.ik2_target_rotations = wp.array(
            [wp.vec4(*GRIPPER_DOWN_QUAT_2)], dtype=wp.vec4, device=self.device
        )
        self.ik2_pos_obj = ik.IKObjectivePosition(
            link_index=ik_tcp_body, link_offset=wp.vec3(*GRIPPER_TCP_LOCAL_OFFSET),
            target_positions=self.ik2_target_positions
        )
        self.ik2_rot_obj = ik.IKObjectiveRotation(
            link_index=ik_tcp_body, link_offset_rotation=wp.quat_identity(),
            target_rotations=self.ik2_target_rotations
        )
        self.ik2_limit_obj = ik.IKObjectiveJointLimit(
            joint_limit_lower=wp.clone(self.ik2_model.joint_limit_lower),
            joint_limit_upper=wp.clone(self.ik2_model.joint_limit_upper),
            weight=10.0
        )
        self.ik2_solver = ik.IKSolver(
            model=self.ik2_model, n_problems=1,
            objectives=[self.ik2_pos_obj, self.ik2_rot_obj, self.ik2_limit_obj],
            lambda_initial=IK_LAMBDA_INITIAL,
            jacobian_mode=ik.IKJacobianType.ANALYTIC
        )
        self.ik2_tcp_body = int(ik_tcp_body)

    def _assign_ik2_target_arrays(self, position, rotation) -> None:
        ...
        if not np.isfinite(pos).all() or not np.isfinite(rot).all() or norm < 1.0e-8:
            raise ValueError(f"Invalid Robot 2 IK target: position={position}, rotation={rotation}")
        rot = rot / norm
        wp.launch(
            _set_single_task_target, dim=1,
            inputs=[
                self.ik2_target_positions, self.ik2_target_rotations,
    ...
    def _initialize_robot2_at_approach(self) -> None:
        self._assign_ik2_target_arrays(BELT_APPROACH_POS_2, GRIPPER_DOWN_QUAT_2)
        self.ik2_solver.step(self.ik2_joint_q, self.ik2_joint_q, iterations=IK_INIT_ITERS)
        solved = self.ik2_joint_q.numpy().reshape(-1)
        if not np.isfinite(solved).all():
            raise RuntimeError("Initial Robot 2 UR10 IK produced NaN/Inf")
        main_q = self.model.joint_q.numpy().copy()
        for main_idx, ik_idx in zip(self.arm2_coord_indices, self.ik2_arm_coord_indices):
            main_q[main_idx] = solved[ik_idx]
        for main_idx, open_val in zip(self.gripper2_coord_indices, self.gripper2_open_values):
            main_q[main_idx] = open_val
        self.model.joint_q.assign(main_q)
        self.model.joint_qd.zero_()
        ik_q = self.ik2_joint_q.numpy().reshape(-1)
        for ik_idx, main_idx in zip(self.ik2_arm_coord_indices, self.arm2_coord_indices):
            ik_q[ik_idx] = main_q[main_idx]
        self.ik2_joint_q.assign(ik_q.reshape(1, -1))
        print("[INFO] Robot 2 t=0 TCP target: "
              f"approach={BELT_APPROACH_POS_2}, grasp={BELT_GRASP_POS_2}")
    ...
    def _measure_actual_gripper2(self):
        ...
        if not self.gripper2_coord_indices:
            return 0.0, 0.0
        jq = self.state_0.joint_q.numpy()
        ...
        for coord_idx, dof_idx, q_open, q_closed in zip(
                self.gripper2_coord_indices, self.gripper2_dof_indices, self.g2_open, self.g2_closed):
        ...
        if not fractions:
            return 0.0, 0.0
        return (float(np.clip(np.median(fractions), 0.0, 1.0)),
    ...
    def _update_gripper2_antcrush(self):
        """Robot 2 version of the same contact latch + stiff post-grasp hold."""
        requested = float(np.clip(self._grip2_requested_fraction, 0.0, 1.0))
        actual, actual_speed = self._measure_actual_gripper2()
        self._grip2_actual_fraction = actual
        self._grip2_actual_speed = actual_speed

        if self._grip2_hold_fraction is not None:
            if requested < self._grip2_hold_fraction - GRIPPER_RELEASE_HYSTERESIS:
                ...
                self._grip2_fraction = min(requested, self._grip2_hold_fraction)
            return

        error = requested - actual
        closing_request = requested > self._grip2_fraction + 1.0e-5
        stalled = (
            closing_request
            and actual >= GRIPPER_STALL_MIN_FRACTION
            and error >= GRIPPER_STALL_ERROR_FRACTION
        ...
        self._grip2_stall_frames = self._grip2_stall_frames + 1 if stalled else 0

        if self._grip2_stall_frames >= GRIPPER_STALL_FRAMES:
            hold = min(requested, actual + GRIPPER_HOLD_PRELOAD_FRACTION)
    ...
    def _write_control_targets2(self, solved_q):
        """Write Robot 2 arm + gripper targets without disturbing Robot 1."""
        targets = self.control.joint_target_q.numpy().copy()
        desired = np.array(
            [float(solved_q[i]) for i in self.ik2_arm_coord_indices], dtype=np.float64
        )
        for j in range(len(desired)):
            desired[j] = self._nearest_equivalent_angle(desired[j], self._arm2_cmd[j])
        arm2_speed_limit = float(self.args.max_arm_speed)
        if self._grip2_hold_fraction is not None:
            arm2_speed_limit = min(arm2_speed_limit, GRASPED_MAX_ARM_SPEED)
        max_step = arm2_speed_limit * self.frame_dt
        delta = np.clip(desired - self._arm2_cmd, -max_step, max_step)
        self._arm2_cmd = self._arm2_cmd + delta
        for ti, cmd in zip(self.arm2_target_indices, self._arm2_cmd):
            targets[ti] = float(cmd)

        frac = float(np.clip(self._grip2_fraction, 0.0, 1.0))
        grip_vals = (
            np.asarray(self.g2_open, dtype=np.float64)
            + frac * (np.asarray(self.g2_closed, dtype=np.float64) - np.asarray(self.g2_open, dtype=np.float64))
        )
    ...
    # Joint config
    def _configure_robot_joints_range(self, start_joint_index: int, end_joint_index: int, label: str) -> dict[str, Any]:
        model = self.model
        ...
        q_start = as_numpy(model.joint_q_start)
        qd_start = as_numpy(model.joint_qd_start)
        robot_coord_start = int(q_start[start_joint_index]) if start_joint_index < len(q_start) else 0
        robot_dof_start = int(qd_start[start_joint_index]) if start_joint_index < len(qd_start) else 0
        robot_coord_end = int(q_start[end_joint_index]) if end_joint_index < len(q_start) else n_coords
        robot_dof_end = int(qd_start[end_joint_index]) if end_joint_index < len(qd_start) else n_dofs
        ...
        ke_np[robot_start:robot_end] = 0.0
        kd_np[robot_start:robot_end] = 0.0
        mode_np[robot_start:robot_end] = int(JointTargetMode.NONE)
    ...
    def _belt_world_shape_pairs(self, include_gripper: bool = False) -> wp.array:
        ...
        pad_shapes = set(self.all_gripper_pad_shapes)
        pairs = []
        n_pulley_pairs = 0
    ...
    def step(self):
        ...
        # Robot 2: identical shared-buffer delta tracking, fully independent.
        raw2_pos, quat2, grip2, ready2 = self.shared2.read()
        if ready2 >= 0.5:
            raw2_pos = np.asarray(raw2_pos, dtype=np.float64)
            quat2 = _norm4(quat2)
            if np.isfinite(raw2_pos).all() and np.isfinite(quat2).all():
                raw2_delta = raw2_pos - self._spacemouse2_raw_pos_prev
                self._spacemouse2_raw_pos_prev = raw2_pos.copy()
                raw2_delta_norm = float(np.linalg.norm(raw2_delta))
                if raw2_delta_norm <= TELEOP_MAX_RAW_TARGET_JUMP:
                    candidate2 = self._target2_pos + raw2_delta
                    bq2 = self.state_0.body_q.numpy()[self.link2_index]
                    base2_pos = np.asarray(bq2[0:3], dtype=np.float64)
                    base2_quat = _norm4(np.asarray(bq2[3:7], dtype=np.float64))
                    current2_tcp = base2_pos + _rotate_vec(base2_quat, self.tip_offset)
                    lead2 = candidate2 - current2_tcp
                    lead2_norm = float(np.linalg.norm(lead2))
                    if lead2_norm > TELEOP_MAX_TARGET_DISTANCE and lead2_norm > 1.0e-12:
                        candidate2 = current2_tcp + lead2 * (TELEOP_MAX_TARGET_DISTANCE / lead2_norm)
                    self._target2_pos = candidate2
                self._target2_xyzw = quat2.copy()
            self._grip2_requested_fraction = float(np.clip(grip2, 0.0, 1.0))

        # Convert both raw SpaceMouse commands into contact-latched applied commands.
        self._update_gripper_antcrush()
        self._update_gripper2_antcrush()
        ...
        # Solve Robot 2 TCP objective. A single IK pass is used here; it has its own
        # seed and command history and does not overwrite Robot 1's targets.
        self.ik2_pos_obj.set_target_position(0, _v3(self._target2_pos))
        self.ik2_rot_obj.set_target_rotation(0, _v4(self._target2_xyzw))
        ik2_seed = self.ik2_joint_q.numpy()
        for j, ik_qi in enumerate(self.ik2_arm_coord_indices):
            ik2_seed[0, ik_qi] = float(self._arm2_cmd[j])
        self.ik2_joint_q.assign(ik2_seed)
        self.ik2_solver.step(self.ik2_joint_q, self.ik2_joint_q, iterations=IK_TRACK_ITERS)
        solved_r2 = self.ik2_joint_q.numpy().reshape(-1)
        if np.isfinite(solved_r2).all():
            self._write_control_targets2(solved_r2)
    ...
    def render(self):
        ...
        # Robot 2 current/target TCP visualization.
        bq2 = self.state_0.body_q.numpy()[self.link2_index]
        base2_pos = np.array(bq2[0:3], dtype=np.float64)
        base2_quat = _norm4(np.array(bq2[3:7], dtype=np.float64))
        tip2 = base2_pos + _rotate_vec(base2_quat, self.tip_offset)
        self._log_frame("/robot2/tip_current", tip2, base2_quat, self.axis_len, self._axis_bright)
        self._log_frame("/robot2/tip_target_axes", self._target2_pos, self._target2_xyzw,
                        self.axis_len, self._axis_dim)
        self._log_sphere("/robot2/tip_target_sphere", self._target2_pos, self.tip_radius)
    ...
    def create_parser():
        ...
        # Independent shared-memory target buffers for the two arms.
        parser.add_argument("--buffer", type=str, default=SHARED_PATH_DEFAULT,
                            help="Robot 1 SpaceMouse shared-memory buffer")
        parser.add_argument("--buffer2", type=str, default=SHARED_PATH_2_DEFAULT,
                            help="Robot 2 SpaceMouse shared-memory buffer")
